...
#define NUM_THREADS 4  // quantidade d ethreads a serem criadas

// Função auxiliar da thread
void* funcaoHelper(void* idThread) {
  sleep(3);
  printf("Finalizar a thread: %d\n", idThread);
  //Finalizar com o cÛdigo de retorno igual ao idThread
...
int main(int argc, char *argv[]){
  ...
  for(int i=0; i < NUM_THREADS; i++){
      ...
      //Verificando a existÍncia de erros na criação
      if (statusRetorno){
